...
void MainWindow::StartRace(void){
    ...
    auto first=[this]{return  this->concurRace1->DoWork(&number, ui->rb_mutexOn->isChecked(), ui->sb_initNum->value()); };
    auto second=[this]{return  this->concurRace2->DoWork(&number, ui->rb_mutexOn->isChecked(), ui->sb_initNum->value()); };

    QFuture<void> fut=QtConcurrent::run(first).then(second);
